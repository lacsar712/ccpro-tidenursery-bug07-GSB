...
from app.time_bounds import as_utc, rolling_window_start
...
def list_events(
    ...
    last_days: Optional[int] = Query(None, alias="lastDays"),
    ...
    if last_days is not None:
        # 与看板汇总共用同一东八区划界函数,保证列表行合计 == 汇总卡
        q = q.filter(FeedEvent.fed_at >= rolling_window_start(last_days))
